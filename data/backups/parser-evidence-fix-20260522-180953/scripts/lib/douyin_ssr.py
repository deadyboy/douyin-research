...
RE_VIDEO_ID = re.compile(r"/video/(\d+)|/share/video/(\d+)")
...
def parse_short_url(short_url: str) -> VideoMeta:
    ...
    if isinstance(video, dict):
        ...
        if isinstance(pa, dict) and "url_list" in pa and pa["url_list"]:
            meta.play_addr = pa["url_list"][0]

        # 无水印 H264
        pa_h264 = video.get("play_addr_h264", {})
        if isinstance(pa_h264, dict) and "url_list" in pa_h264 and pa_h264["url_list"]:
            # 优先用 h264 作为主播放地址
            meta.play_addr = pa_h264["url_list"][0] or meta.play_addr

        # 有水印（备用）
        pa_wm = video.get("play_addr", {})
        if isinstance(pa_wm, dict) and "url_list" in pa_wm and pa_wm["url_list"]:
            meta.play_addr_watermark = pa_wm["url_list"][0]

        # download_addr 作为最终兜底
        if not meta.play_addr or "playwm" in meta.play_addr:
            da = video.get("download_addr", {})
            if isinstance(da, dict) and "url_list" in da and da["url_list"]:
                # 尝试找无水印版本
                for url in da["url_list"]:
                    if "playwm" not in url:
                        meta.play_addr = url
                        break
                if not meta.play_addr or "playwm" in meta.play_addr:
                    meta.play_addr = da["url_list"][0]
